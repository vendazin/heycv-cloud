#!/usr/bin/env python3
"""heycv.cloud – Backend Server (lightweight, no PDF processing)"""
...
def optimize():
    ...
    # CV text comes directly from frontend (PDF parsed in browser)
    cv_text = (request.form.get("cvText") or "").strip()
    job_text = (request.form.get("jobText") or "").strip()
    job_url  = (request.form.get("jobUrl") or "").strip()
    opts_raw = request.form.get("options", "{}")

    # Also accept DOCX files (browser can't parse those)
    cv_file = request.files.get("cv")
    if cv_file and not cv_text:
        fn = (cv_file.filename or "").lower()
        if fn.endswith((".docx", ".doc")):
            try:
                cv_text = parse_docx(cv_file.read())
            except Exception as e:
                return jsonify({"error": f"Word-Datei konnte nicht gelesen werden: {e}"}), 422
        else:
            try:
                cv_text = cv_file.read().decode("utf-8", errors="replace")
            except Exception:
                pass
    ...
    if not cv_text:
        return jsonify({"error": "Kein Lebenslauftext empfangen."}), 400
    ...
    prompt = build_prompt(cv_text[:12000], job_text, opts)
    try:
        resp = requests.post(
